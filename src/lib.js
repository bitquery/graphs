...
export function address_graph(selector, dataSource, displayedData, options) {
  ...
  const jqContainer = $(g.container)
    ...
    ? {
        [displayedData.split('.')[1]]: dataSource.values,
      }
...
export function addControls(selector, query, options) {
  ...
  controls.createBottomMenu = (fullScreen = true, mJS = true, mGQL = true) => {
    const menu = $(`<div class="graph-bottom-menu"></div>`)
    // jqContainer.append(menu)
    jqWrapper.append(menu)

    fullScreen ? addFullScreenButton(menu, jqWrapper) : null

    const buttonsBlock = $(`<div class="graph-bottom-menu__buttons"></div>`)
    menu.append(buttonsBlock)

    mJS ? addModalJS(buttonsBlock, query.JSCode, options, query) : null
    mGQL ? addModalGraphQL(buttonsBlock, options, query) : null
  }
  ...
  $('body').on('DOMSubtreeModified', selector, function render() {
    // options.buttons = ['currencyFilter', 'detailLevel', 'depthLevel', 'fullScreen', 'mJS', 'mGQL']
    if (options.buttons.includes('currencyFilter')) {
      controls.setCurrency()
      controls.currencyFilter()
    }
    if (options.buttons.includes('detailLevel')) {
      controls.detailLevel()
    }
    if (options.buttons.includes('depthLevel')) {
      controls.depthLevel()
    }
    controls.createBottomMenu(
      options.buttons.includes('fullScreen'),
      options.buttons.includes('mJS'),
      options.buttons.includes('mGQL')
    )
